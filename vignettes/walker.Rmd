...
library("rstan")
stan_plot(kalman_walker, pars = "sigma")
```
...
Using the `extract` method of `rstan` we can pick up the samples corresponding to $\beta$'s, and we can for example plot the posterior mean paths with `ts.plot` (the dashed lines correspond to true coefficients), or uncertainty intervals:
